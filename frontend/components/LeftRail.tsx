...
export function LeftRail({
  ...
  return (
    <>
      ...
      <nav className="z-20 hidden w-[88px] shrink-0 flex-col items-center gap-1.5 border-r border-line bg-white py-4 lg:flex">
      ...
      <nav className="pb-safe fixed inset-x-0 bottom-0 z-30 flex items-stretch justify-between border-t border-line bg-white/95 px-1 shadow-[0_-4px_16px_-8px_rgba(15,23,42,0.12)] backdrop-blur-xl lg:hidden">
        {DASHBOARD_MODULES.map((m) => {
          const isActive = active === m.id;
          return (
            <button
              ...
              aria-label={m.name}
              aria-pressed={isActive}
              className={`relative flex min-h-[56px] flex-1 flex-col items-center justify-center gap-1 px-1 py-2 text-[10px] font-medium transition-colors ${
                isActive ? m.accent : "text-ink-subtle"
              }`}
            >
              {isActive && (
